...
- Header: wordmark solda, linkler (Services, Expertise, Approach, Insights, Contact), EN/TR, "Book a call" → `#contact`.
...
Cloudflare `styles.css` ve `demo.js`'i cache'ler. CSS/JS değişince **her HTML'de** `?v=N` sürümünü artır (`index.html`, `tr/index.html`, `insights/**/index.html`; build_insights.py içindeki `styles.css?v=` de). Şu an **v=10**. Artırmazsan canlıda eski CSS + yeni HTML görünür ("site bozuldu" şikayetinin sebebi buydu).
